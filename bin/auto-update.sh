#!/bin/bash
#
# This script was copied from http://krl.io/5e8d1
# The changes made were to customize it for my test site, and add comments for clarity.

# The multidev name set here is deleted if it already exists, and then created to run our tests in.
MULTIDEV="update-wp"
# It appears that the sitename set here is only used in the Slack messages, but isn't required for basic functionality.  
SITENAME="YourAwesomeSite"
...
# Now we see if any updates were applied. If not, we let Slack now and finish. Otherwise...
if [[ "${UPDATES_APPLIED}" = false ]]
then
    # no updates applied
    echo -e "\nNo updates to apply..."
    SLACK_MESSAGE="${SITENAME} Circle CI update check #${CIRCLE_BUILD_NUM} by ${CIRCLE_PROJECT_USERNAME}. No updates to apply, nothing deployed."
...
# Otherwise we start testing for visual differences.
else
    ...
    # Here, I've modified the script to use a variable, $SITE_URL, instead of the original hardcoding for wp-microsite.pantheonsite.io
    echo -e "\nPinging the ${MULTIDEV} multidev environment to wake it from sleep..."
    curl -I "https://$MULTIDEV-${SITEURL}"
